...
class CRUDOrderServiceProduct(CRUDBase[OrderServiceProducts, OrderServiceProductsCreate, OrderServiceProductsUpdate]):
    ...
    def get_by_id(self, db: Session, *, order_service_product_id: uuid_UUID) -> Optional[OrderServiceProducts]:
        db_obj = db.query(OrderServiceProducts).filter(OrderServiceProducts.id == order_service_product_id).first()
        if not db_obj:
            raise HTTPException(status_code=400, detail="The order_service_product connection doesn't exist")
        return db_obj

    def get_by_order_id_sp_id(
        self, db: Session, *, order_id: uuid_UUID, service_product_id: uuid_UUID, with_lock=False
    ) -> Optional[OrderServiceProducts]:
        if not with_lock:
            db_obj = (
                db.query(OrderServiceProducts)
                .filter(
    ...
    def update(
        self, db: Session, *, db_obj: OrderServiceProducts, obj_in: Union[OrderServiceProductsUpdate, Dict[str, Any]]
    ) -> OrderServiceProducts:
    ...
    def filter_by_order_ids(self, db: Session, *, order_ids: list):
        return (
            db.query(ServiceProduct.title, functions.sum(OrderServiceProducts.price))
            .filter(
            ...
            .group_by(OrderServiceProducts.service_product_id, ServiceProduct.title)
            .all()
        )
