...
from app.tests.utils.user import create_random_user, create_random_user_with_balance
from app.tests.utils.utils import random_lower_string


def test_create_balance_zero(client: TestClient, db: Session):
    ...
    assert db_balance.amount == 0.0
    assert db_balance.amount_reserved == 0.0


def test_create_balance_non_zero(client: TestClient, db: Session):
    # valid user_id and invalid user_id
    user = create_random_user(db)
    amount = round(random.uniform(1.0, 1000.0), 2)
    data = {"user_id": str(user.id), "amount": amount}
    r = client.post(
        f"{settings.API_V1_STR}/balance/",
        json=data,
    )
...
def test_balance_to_balance_transaction_enough_funds(client: TestClient, db: Session):
    ...
    assert db_balance_from.amount_reserved == 0.0
    assert db_balance_to.amount_reserved == 0.0


def test_balance_to_balance_transaction_not_enough_funds(client: TestClient, db: Session):
    amount = round(random.uniform(1.0, 1000.0), 2)
    ...
    assert db_balance_2.amount == 0.0
    assert db_balance_2.amount_reserved == 0.0


def test_add_to_balance(client: TestClient, db: Session):
    amount = round(random.uniform(1.0, 1000.0), 2)
    user, balance = create_random_user_with_balance(db, amount=amount)
    amount_to_add = round(random.uniform(1.0, 1000.0), 2)
    data = {"user_id": str(user.id), "amount": amount_to_add}
    r = client.post(
        f"{settings.API_V1_STR}/balance/moneyIn",
        json=data,
    )
    crud.balance.close_session(db)
    assert 200 == r.status_code
...
def test_withdraw_from_balance(client: TestClient, db: Session):
    amount = round(random.uniform(1.0, 1000.0), 2)
    user, balance = create_random_user_with_balance(db, amount=amount)
    amount_to_withdraw = round(amount / random.uniform(2.0, 100.0), 2)
    data = {"user_id": str(user.id), "amount": amount_to_withdraw}
    r = client.post(
        f"{settings.API_V1_STR}/balance/moneyOut",
        json=data,
    )
    crud.balance.close_session(db)
    db_balance = crud.balance.get_by_user_id(db, user_id=user.id)
...
def test_reserve_enough_funds(client: TestClient, db: Session):
    sp_id, sp_price, user, balance, transaction, tr_event, order, ospl, data = arrange_for_balance_reserve(client, db)
    initial_amount, initial_amount_reserved = balance.amount, balance.amount_reserved
    order_id, order_amount = order.id, order.amount

    r = client.post(
        f"{settings.API_V1_STR}/balance/reserve",
        json=data,
    ...
    tr_event_link = crud.transaction_event.get_by_transaction_id(db, transaction_id=transaction.id)
...
def test_reserve_capture(client: TestClient, db: Session):
    ...
    r = client.post(
        f"{settings.API_V1_STR}/balance/reserve/capture",
        json=data,
    )
    assert 200 == r.status_code
...
def test_reserve_refund_authorised(client: TestClient, db: Session):
    ...
    r = client.post(
        f"{settings.API_V1_STR}/balance/reserve/refund",
        json=data,
    )
    assert 200 == r.status_code
    api_balance, api_osp = r.json()
