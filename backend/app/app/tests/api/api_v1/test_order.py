...
def test_create_multi(client: TestClient, db: Session):
    ...
    db_order = crud.order.get_by_id(db, order_id=api_order["id"])
    db_osps = [
        crud.order_service_products.get_by_order_id_sp_id(
            db, order_id=api_order["id"], service_product_id=api_osps[i]["service_product_id"]
        )
    ...
    assert db_osps
    db_osps_sorted = sorted(
        [(db_osps[i].service_product_id, db_osps[i].order_id, db_osps[i].price) for i in range(sp_num)],
        key=lambda x: x[0],
    )
    api_osps_sorted = sorted(
        [
            (UUID(api_osps[i]["service_product_id"]), UUID(api_osps[i]["order_id"]), api_osps[i]["price"])
            for i in range(sp_num)
        ],
        key=lambda x: x[0],
    )
    service_products_sorted = sorted(
        [(UUID(service_products[i]["id"]), UUID(api_order["id"]), service_products[i]["price"]) for i in range(sp_num)],
        key=lambda x: x[0],
    )
